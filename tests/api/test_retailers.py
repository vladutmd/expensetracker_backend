...
@pytest.mark.django_db
def test_get_retailers_with_token_actual_categories(api_client: APIClient, user_factory, retailer_factory):
...
def test_post_retailer_without_authentication(api_client):
    ...
    list_response: Response = api_client.post(list_retailer_url, data=data, format="json")
    assert list_response.status_code == status.HTTP_401_UNAUTHORIZED
...
def test_post_retailer_twice(api_client, user_factory):
    ...
    second_list_response: Response = api_client.post(list_retailer_url, data=data, format="json")
    ...
    assert "There is already a Retailer with this name for the current user" in json_response["non_field_errors"]
    assert "There is already a Retailer with this slug for the current user" in json_response["non_field_errors"]
